...
export const signUp = (payload) => (dispatch) =>
    axios
        .post("https://zebra-hackathon.herokuapp.com/api/register", {
            ...payload,
        })
        .then((res) => {
...
export const signIn = (payload) => (dispatch) =>
    axios
        .post("https://zebra-hackathon.herokuapp.com/api/login", { ...payload })
        .then((res) => {
            if (res.status !== 200) {
            ...
            dispatch(
                setCredentialsAction({ user: data, token: res.data.token })
            );
        });
